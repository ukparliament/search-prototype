  ...
  describe 'search link' do
    ...
    context 'when given a SES ID & field name' do
      it 'returns a link to a new search using the SES ID as a filter for the given field' do
        ...
        allow(helper).to receive(:ses_data).and_return(mock_ses_data)
        expect(helper.search_link(input_data_ses)).to eq("<a href=\"/search-prototype/search?filter%5Bfield_name%5D=member_ses&amp;filter%5Bvalue%5D=123\">John Smith</a>")
      end
    end
    context 'when given a string value and a field name' do
      it 'returns a link to a new search using the string as a filter for the given field' do
        expect(helper.search_link(input_data_string)).to eq("<a href=\"/search-prototype/search?filter%5Bfield_name%5D=memberPrinted_t&amp;filter%5Bvalue%5D=John+Smith\">John Smith</a>")
      end
    end
  end
  ...
  describe 'object_display_name_link' do
    ...
    context 'when given a SES ID and field name' do
      it 'returns a link to search that SES field with the given ID' do
        # requires SES data to have been preloaded on the page - this is done for performance reasons
        allow(helper).to receive(:ses_data).and_return(mock_ses_data)
        expect(helper.object_display_name_link(input_data_ses)).to eq("<a href=\"/search-prototype/search?filter%5Bfield_name%5D=type_ses&amp;filter%5Bvalue%5D=123\">Early day motion</a>")
      end
    end
    context 'when given string and field name' do
      it 'returns a link to search using the string as a query' do
        expect(helper.object_display_name_link(input_data_string)).to eq("<a href=\"/search-prototype/search?filter%5Bfield_name%5D=type_t&amp;filter%5Bvalue%5D=Early+day+motions\">Early day motion</a>")
      end
    end
    context 'when called with singularisation disabled' do
      it 'returns a link to search using the string as a query, and a plural item name' do
        expect(helper.object_display_name_link(input_data_string, singular: false)).to eq("<a href=\"/search-prototype/search?filter%5Bfield_name%5D=type_t&amp;filter%5Bvalue%5D=Early+day+motions\">Early day motions</a>")
      end
    end
  end
