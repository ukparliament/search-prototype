...
RSpec.describe 'SesQuery' do
  ...
  describe 'data' do
    ...
    context 'where a term is submitted' do
      ...
      let!(:formatted_query) { URI("https://api.parliament.uk/ses/ses?TBDB=disp_taxonomy&TEMPLATE=service.json&expand_hierarchy=0&SERVICE=search&QUERY=housing") }

      it 'returns a hash containing equivalent terms, perferred term, preferred term ID and topic ID' do
        expect(ses_query.data.keys).to eq([:equivalent_terms, :preferred_term_id, :preferred_term, :topic_id])
        expect(ses_query.data[:equivalent_terms]).to eq([["Accommodation", "Houses"]])
        expect(ses_query.data[:preferred_term]).to eq("Housing")
        expect(ses_query.data[:preferred_term_id]).to eq("91569")
        expect(ses_query.data[:topic_id]).to eq("95629")
      end
    end
  end
end
