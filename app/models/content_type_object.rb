class ContentTypeObject
  ...
  attr_reader :content_type_object_data
  ...
  def page_title
    content_type_object_data['title_t']
  end
  ...
  def get_as_string_from(field_name)
    return if content_type_object_data[field_name].blank?

    { value: content_type_object_data[field_name], field_name: field_name }
  end

  def get_first_as_boolean_from(field_name)
    return unless ['true', 'false', true, false].include?(content_type_object_data[field_name]&.first)

    result = ['true', true].include?(content_type_object_data[field_name].first) ? true : false
  ...
  def get_first_from(field_name)
    ...
    { value: content_type_object_data[field_name].first, field_name: field_name }
  end

  def get_first_id_from(field_name)
    return if content_type_object_data[field_name].blank?

    content_type_object_data[field_name].first
  end

  def get_all_ids_from(field_name)
    return if content_type_object_data[field_name].blank?

    content_type_object_data[field_name]
  end

  def get_first_as_html_from(field_name)
    return if content_type_object_data[field_name].blank?

    { value: CGI::unescapeHTML(content_type_object_data[field_name].first), field_name: field_name }
  end

  def get_as_html_from(field_name)
    return if content_type_object_data[field_name].blank?

    { value: CGI::unescapeHTML(content_type_object_data[field_name]), field_name: field_name }
  end
  ...
  def get_first_as_date_from(field_name)
    ...
    valid_date_string = validate_datetime(content_type_object_data[field_name].first)
  ...
  def get_as_date_from(field_name)
    ...
    valid_date_string = validate_datetime(content_type_object_data[field_name])
    return unless valid_date_string
